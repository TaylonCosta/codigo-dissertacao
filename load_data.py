...
class Load_data:
    ...
    def load(self, instance):
        ...
        horas_D14 = [
            f"{dia}_{hora}" for dia in dias for hora in horas
        ]  # d01_h01, d01_h02, ...   , d14_h6
        horas_Dm3 = [
            f"dm{dia+1:02d}_h{hora+1:02d}"
            for dia in range(-4, -1)
            for hora in range(len(horas))
        ]  # dm-3_h01, dm-3_h02, ...   , dm-1_h6
        ...
        de_para_produtos_mina_conc = {
            "PRDT1": {"PRDT_C1": 1, "PRDT_C2": 1, "PRDT_C3": 0},
            "PRDT2": {"PRDT_C1": 0, "PRDT_C2": 0, "PRDT_C3": 1},
        }
        de_para_produtos_conc_usina = {
            "PRDT_C1": {"PRDT_U1": 0, "PRDT_U2": 0, "PRDT_U3": 0, "PRDT_U4": 1},
        ...
        taxa_producao_britagem = {
            dias[d]: taxa_alimentacao_britagem[d]
            * disponibilidade_britagem[d]
            / 100
            * utilizacao_britagem[d]
            / 100
        ...
        produtos_britagem = {
            produto: {hora: 0 for hora in horas_D14} for produto in produtos_mina
        }
        campanha_atual = 0
        for hora in horas_D14:
            if (
                campanha_atual < len(campanha_c3)
                and hora == campanha_c3[campanha_atual][1]
            ):
        ...
        for produto, estoque in cenario["concentrador"][
            "estoque_pulmao_inicial_concentrador"
        ]:
            estoque_pulmao_inicial_concentrador[produto] = estoque
        min_estoque_pulmao_concentrador = cenario["concentrador"][
            "min_estoque_pulmao_concentrador"
        ]
        max_estoque_pulmao_concentrador = cenario["concentrador"][
            "max_estoque_pulmao_concentrador"
        ]
        faixas_producao_concentrador = cenario["concentrador"][
            "faixas_producao_concentrador"
        ]
        ...
        # Usina
        max_producao_sem_incorporacao = cenario["usina"][
            "max_producao_sem_incorporacao"
        ]
        ...
        capacidade_carreg_porto_por_dia = cenario["porto"][
        ...
        for dia in dias:
            parametros_calculados["RP (Recuperação Mássica) - C3"][dia] = cenario[
                "mina"
            ]["RP"][dia]
        ...
        for dia in dias:
            parametros_calculados["% Sólidos - EB06"][dia] = (
                cenario["mina"]["SOL"][dia] - 0.012
            )
            parametros_calculados["Densidade Polpa - EB06"][dia] = 1 / (
                (cenario["mina"]["SOL_EB06"][dia] / 4.75)
                + (1 - cenario["mina"]["SOL_EB06"][dia])
            )
            parametros_calculados["Relação: tms - EB06"][dia] = (
                5395
                * cenario["mina"]["SOL_EB06"][dia]
                * cenario["mina"]["densidade"][dia]
                / 100
            )
            parametros_calculados["Densidade Polpa - EB07"][dia] = cenario["mina"][
                "densidade"
            ][dia]
        ...
        bomb_polpa_acum_semana_anterior = cenario["mineroduto"][
            "bombeamento_polpa_acum_semana_anterior"
        ]
        bomb_agua_acum_semana_anterior = cenario["mineroduto"][
            "bombeamento_agua_acum_semana_anterior"
        ]
        ...
        data = {
            "horas_D14": horas_D14,
            "produtos_conc": produtos_conc,
            "horas_Dm3_D14": horas_Dm3_D14,
            "de_para_produtos_mina_conc": de_para_produtos_mina_conc,
            "min_estoque_pulmao_concentrador": min_estoque_pulmao_concentrador,
            "max_estoque_pulmao_concentrador": max_estoque_pulmao_concentrador,
            "numero_faixas_producao": numero_faixas_producao,
            "max_taxa_alimentacao": max_taxa_alimentacao,
            "parametros_mina": parametros_mina,
            "taxa_producao_britagem": taxa_producao_britagem,
            "produtos_britagem": produtos_britagem,
            "produtos_mina": produtos_mina,
            ...
            "dias": dias,
            "args": args,
            "max_producao_sem_incorporacao": max_producao_sem_incorporacao,
            "produtos_usina": produtos_usina,
            "de_para_produtos_conc_usina": de_para_produtos_conc_usina,
            "parametros_ubu": parametros_ubu,
            "tempo_mineroduto": tempo_mineroduto,
            "min_estoque_polpa_ubu": min_estoque_polpa_ubu,
            "max_estoque_polpa_ubu": max_estoque_polpa_ubu,
            "max_taxa_envio_patio": max_taxa_envio_patio,
            "max_taxa_retorno_patio_usina": max_taxa_retorno_patio_usina,
            "min_estoque_patio_usina": min_estoque_patio_usina,
            "max_estoque_patio_usina": max_estoque_patio_usina,
            ...
            "horas_Dm3": horas_Dm3,
            "navios": navios,
            "vazao_bombas": vazao_bombas,
            "AguaLi": AguaLi,
            "AguaLs": AguaLs,
            "PolpaLi": PolpaLi,
            "PolpaLs": PolpaLs,
            "carga_navios": carga_navios,
            "taxa_carreg_navios": taxa_carreg_navios,
            "estoque_produto_patio": estoque_produto_patio,
            "capacidade_carreg_porto_por_dia": capacidade_carreg_porto_por_dia,
            "produtos_navio": produtos_de_cada_navio,
            "capacidade_patio_porto_min": capacidade_patio_porto_min,
            "capacidade_patio_porto_max": capacidade_patio_porto_max,
            "data_chegada_navio": data_chegada_navio,
            "perc_solidos": perc_solidos,
            "densidade": densidade,
            "DF": DF,
            "UD": UD,
            "umidade": umidade,
            ...
            "bomb_polpa_acum_semana_anterior": bomb_polpa_acum_semana_anterior,
            "bomb_agua_acum_semana_anterior": bomb_agua_acum_semana_anterior,
            "max_capacidade_eb06": max_capacidade_eb06,
            "tempo_germano_matipo": tempo_germano_matipo,
            "tempo_germano_ubu": tempo_germano_ubu,
            "prod_bomb_hora_anterior": prod_bomb_hora_anterior,
            "fator_conv": fator_conv,
            "prod_polpa_hora_anterior": prod_polpa_hora_anterior,
            "min_producao_produtos_ubu": min_producao_produtos_ubu,
            "prod_para_estoque": prod_para_estoque,
            "janelas_campanhas_min": janelas_campanhas_min,
            "janelas_campanhas_max": janelas_campanhas_max,
            "janelas_campanha_acum": janelas_campanha_acum,
            "limites_campanhas_min": limites_campanhas_min,
            "limites_campanhas_max": limites_campanhas_max,
            "limites_campanhas_acum": limites_campanhas_acum,
        }
    ...
    def load_simplified_data_ppo(self, instance):
        ...
        disp_conc_inicial = (
            (
                cenario["concentrador"]["max_taxa_alimentacao"]
                * (1 - cenario["mina"]["umidade"]["d01"])
                * (cenario["mina"]["RP"]["d01"] / 100)
                * cenario["mina"]["DF"]["d01"]
                * (1 - cenario["mina"]["dif_balanco"]["d01"])
            )
            * (1 / cenario["mina"]["SOL"]["d01"])
            * (1 / cenario["mina"]["densidade"]["d01"])
        )
